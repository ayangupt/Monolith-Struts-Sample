...
- Implement Ski Shop EC as a **monolithic** application (integrate microservices into a single WAR).
...
- Design approach: **Layered Architecture** + **Orthodox GoF/EE Patterns**
  ...
  - DAO + DTO/ValueObject
  - Template Method (DAO common processing)
...
| Taglib | struts-html/bean/logic/tiles | 1.2.x | View operations |
| JDBC | commons-dbcp | 1.2.x | Connection pooling |
| JDBC helper | commons-dbutils | 1.1 | Query helper (optional) |
| Logging | log4j | 1.2.17 | Logging |
| Email | JavaMail | 1.4.7 | Notification emails |
...
- **Service Facade**: `*Service` provides use case-level APIs
- **Facade**: `OrderFacade` aggregates complex use cases like order processing
- **Business Delegate**: Mediates Action → Service (reduces coupling)
- **DAO**: Encapsulates DB access, SQL management
- **DTO/VO**: Display data objects passed to JSP
- **Template Method**: `AbstractDao` for common processing (connection/exception/close)
- **Intercepting Filter**: Authentication & authorization check (`AuthRequestProcessor`)

## 5. Module Configuration (Microservices Integration)
...
> ※ Tables are shared within the monolith (unified schema). Add indexes as needed.
...
### Add to Cart & Confirmation & Checkout
...
## 10. Security/Session/Exceptions

- **Authentication**: `AuthRequestProcessor` checks `request.getSession(false)`, redirects to `/login.do` if not logged in.
- **Authorization**: Custom attributes (`roles`) on Actions, checked by `AuthRequestProcessor` (ActionMapping extension).
- **CSRF**: Struts Token (`saveToken`, `isTokenValid`)
- **Input validation**: Struts Validator (`validation.xml`) + `LoginForm.validate()`
- **Exception handling**: `global-exceptions` → `error.jsp`, log output to log4j
- **Session maintenance**: `Cart` in session or DB (cart_id in cookie)

## 11. DAO/Service Implementation Guidelines (Samples)
...
- **Service/DAO**: JUnit + H2 (HSQLDB) in-memory, fixed data with DBUnit
...
- **Connection pooling**: commons-dbcp (maxActive, maxIdle settings)
- **Caching**: Simple LRU (`LinkedHashMap`) or EHCache 1.x (optional)
- **Monitoring**: Apache Access Log, log4j RollingFileAppender
- **Internationalization**: `messages.properties`, `messages_ja.properties`
- **Configuration**: `WEB-INF/classes/app.properties` (DB connection strings, etc.)

---
This design document uses only Java 1.5 / Struts 1.2.9 era syntax and configuration guidelines. Modern features (annotations, DI frameworks, lambdas, etc.) are not used at all.

## 14. GoF Pattern Application List

| Pattern | Purpose | Implementation Example |
| --- | --- | --- |
| Facade | Aggregate complex use cases | `OrderFacade` |
...
| Strategy | Price calculation/coupon application/points calculation | `PricingStrategy`, `CouponRuleEvaluator` |
| Adapter | External payment/auth integration | `StripePaymentAdapter` (example) |
...
| Command | Per-request processing | Each `Action` |
| DAO | Persistence encapsulation | `UserDao`, `OrderDao` |
| DTO/VO | Display/transfer objects | `ProductDto`, `OrderDto` |
| Observer (manual) | Domain event notification | Points award notification, log output |
| Abstract Factory | DAO creation switching | `DaoFactory` (room for extension) |
...
| `/login.do` | GET/POST | LoginAction | loginForm | success=`/home.do`, failure=`/WEB-INF/jsp/auth/login.jsp` | No | Session updated on successful login
| `/register.do` | GET/POST | RegisterAction | registerForm | success=`/login.do`, failure=`/WEB-INF/jsp/auth/register.jsp` | No | User registration (no duplicate emails)
| `/password/forgot.do` | POST | PasswordForgotAction | passwordResetRequestForm | success=`/WEB-INF/jsp/auth/password/forgot_complete.jsp`, failure=`/WEB-INF/jsp/auth/password/forgot.jsp` | No | Email sent
...
| `/logout.do` | GET | LogoutAction | - | success=`/home.do` | Yes | Session invalidate then home
...
| `/cart.do` | GET/POST | CartAction | addCartForm (scope=session) | success=`/WEB-INF/jsp/cart/view.jsp` | No | POST to add, GET to view
...
| `/coupon/apply.do` | POST | CouponApplyAction | couponForm | success=`/WEB-INF/jsp/cart/view.jsp`, failure=`/WEB-INF/jsp/cart/view.jsp` | Yes | Works with cart
...
| `/addresses.do` | GET | AddressListAction | - | input=`/WEB-INF/jsp/account/addresses.jsp` | Yes | Address book display
| `/addresses/save.do` | POST | AddressSaveAction | addressForm | success=`/addresses.do`, failure=`/WEB-INF/jsp/account/address_edit.jsp` | Yes | 10 address limit
| `/admin/*` | - | AdminActions | - | admin JSPs | Yes+ROLE=admin | ActionMapping extension specifies roles
...
| CheckoutForm | cartId, couponCode, paymentMethod, cardNumber, cardExpMonth, cardExpYear, cardCvv, billingZip | String | Yes | card Luhn, month 1-12, year>=current, CVV=3-4 digits | Payment info is ephemeral (not persisted) |
...
| AddressForm | id, label, recipientName, postalCode, prefecture, address1, address2, phone, isDefault | String, boolean | Yes | postalCode mask, phone mask | 10 address limit |
...
| session | `cart` | `CartDto` | CartAction | Read-only reference, updates via service |
...
| request | `error` | `String` or `ActionMessages` | Each Action | Displayed via `<html:errors/>` |
| cookie | `CART_ID` | String(UUID) | CartAction | Expiration: 30 days |

## 18. Transaction/Consistency

- **Transaction start/end**: Service layer (e.g., `OrderService.placeOrder`) with `autoCommit=false`, `commit` on success, `rollback` on exception.
- **Isolation level**: READ_COMMITTED (DB setting), pessimistic lock with `SELECT ... FOR UPDATE` for inventory updates.
- **Order confirmation flow**:
  1. Get cart & validate
  2. Verify coupon (usage count/period/minimum amount)
  3. Reserve inventory (reserved_quantity += qty, exception if insufficient)
  4. Payment authorization (external integration throws exception on failure, holds intent_id on success)
  5. Save order (orders, order_items)
  6. Award points (update point_accounts, add point_transactions)
  7. Clear cart, commit
  8. Capture payment after commit (retry if needed)
- **Consistency guarantee**: If DB fails after payment success, perform payment void/cancel processing.

- **Cancel/Return flow**:
  - Cancel: `status=CREATED/CONFIRMED` only, return reserved inventory, payment void/refund, deduct points, decrement coupon usage count.
  - Return: `status=DELIVERED`, return quantity <= order quantity, return inventory (optional), record refund transaction, adjust points.

## 19. Status/Constants Definition (enum recommended)
...
- Amounts: `BigDecimal` scale=2, rounding=HALF_UP, currency: `USD` assumption (room for extension)
- Points: 1pt = $1, award rate=1% (floor(total_amount)), expiration=365 days
- Tax: Tax rate=10%, `tax = round(subtotal * 0.1)`
- Shipping: $8 (pre-tax)*, free for orders over $100 (*tax applied for tax-inclusive calculations)
...
- `coupons.code` UNIQUE
- `user_addresses(user_id, is_default)` partial UNIQUE (1 row only)
...
- **CSRF**: `saveToken(request)` on form rendering, `isTokenValid` verification on POST, `resetToken` on success.
- **XSS**: JSP uses `<bean:write filter="true"/>` by default, input values are re-escaped.
- **Cookie**: `CART_ID` is HttpOnly (set via response header), Secure (HTTPS recommended).
- **Login attempt limit**: Lock after 5 failures `users.status=LOCKED`, record to `security_logs`.
- **Password reset**: 1-hour valid token stored in `password_reset_tokens`, single use, reset URL in email body.
...
## 24. ServiceLocator / Factory (outline)
...
## 25. Points/Coupon Calculation Rules
...
- **Coupon**:
  ...
  - Application order: Coupon → Points → Tax & shipping calculation (tax-inclusive price)

## 26. Logging/Monitoring

- **MDC**: `reqId` generated from `X-Request-Id` header or UUID, `MDC.put("reqId", ...)`.
- **Access log**: Use Tomcat AccessLogValve.
- **Monitoring**: Log rotation, deadlock detection via thread dump procedures documented in runbook.

## 27. Test Data (samples)
...
('addr-1','u-1','Home','John Smith','10001','NY','123 Main St','Apt 101','2125551234',true,NOW(),NOW());
```

## 28. Coding Conventions/Other

- **Naming**: Classes `PascalCase`, methods/variables `camelCase`, constants `UPPER_SNAKE_CASE`.
- **Exceptions**: DAO wraps in `DaoException`, Action packages into `ActionMessages` and forwards.
- **DTO**: Only DTOs passed to JSP (don't pass domain models directly).
- **Time**: All stored in UTC, localized on display.
...
| --- | --- | --- | --- |
| Member | Member registration/login/logout | Supported | No duplicate emails on registration, password 8+ chars |
| Member | Password reset | Supported | 1-hour token, email sent |
| Member | Profile edit | Supported | username only |
| Member | Address book | Supported | 10 address limit, 1 default |
| Search | Product search/category/sorting | Supported | keyword, categoryId, priceAsc/Desc/newest |
| Inventory | Inventory display/reservation | Supported | reserved_quantity, pessimistic lock |
...
| Shipping | Shipping fee calculation/address specification | Supported | $8/$100+ free, address book selection |
| Tax | Sales tax calculation | Supported | 10% |
